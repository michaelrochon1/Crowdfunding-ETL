...
SELECT con.first_name, con.last_name, con.email, (cpg.goal - cpg.pledged) as remaining_goal 
INTO contacts_live_projects
FROM campaign as cpg
INNER JOIN contacts as con
ON (cpg.contact_id = con.contact_id)
WHERE (cpg.outcome = 'live')
ORDER BY contacts_live_projects DESC;

-- Check the table

SELECT * FROM contacts_live_projects;
